...
let input_j _init mapping json =
...
let vars init mapping query =
  let (_,vars,_,_) = Derive.derive mapping query in
  print_atd @@ Atdgen.of_vars ~init vars
...
let () =
  ...
  let map_query f l =
    ...
    let f mapping query =
      let init = match !init with None -> [] | Some fname -> Atdgen.parse_file fname in
      printfn "(* Generated by esgg from %s based on %s *)" query mapping;
      f init { Common.mapping = json mapping; name = !name } (template query)
    in
