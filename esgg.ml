...
let input_j ~debug ~header _init mapping json =
  let (query,vars,map,(action,url,args,body)) = Derive.derive mapping json in
  match debug with
  | true -> printfn "%s" (match body with Some json -> Tjson.show json | None -> "None")
  | false ->
  printfn "%s" header;
...
let vars ~header inits mapping query =
  let (_,vars,_,_) = Derive.derive mapping query in
  let atd = Atdgen.of_vars ~inits vars in
  print_endline header;
  print_atd atd

let output ~header inits mapping query =
  let shape = Derive.output mapping query in
  let atd = Atdgen.of_shape ~inits "result" shape in
  print_endline header;
  print_atd atd

let atd ~header inits mapping query =
  let (_,vars,_,_) = Derive.derive mapping query in
  let shape = Derive.output mapping query in
  let atd = Atdgen.make ~inits vars "result" shape in
  print_endline header;
  print_atd atd
...
let help () = [
  ...
  "  shared # print the built-in shared atd (currently: _shards, failure, reason) that esgg references in every generated atd";
  "  tjson lift <file.json> # lift json template to code";
  "  tjson decode <file.json> # debug json template parsing";
  "  tjson show <file.json> # parse and print back json template";
] |> List.iter print_endline

let builtin_shared = Atdgen.parse_string Esgg_shared.content
...
let () =
  ...
  let map_query mode f l =
    let user_shared = ref None in
    let name = ref None in
    let f mapping query =
      let inits = builtin_shared :: (match !user_shared with None -> [] | Some fname -> [Atdgen.parse_file fname]) in
      let header = sprintf "(* Generated by esgg %s from %s based on %s *)" mode query mapping in
      f ~header inits { Common.mapping = json mapping; name = !name } (template query)
    in
    let rec loop = function
    | "-shared"::s::tl -> user_shared := Some s; loop tl
  ...
  | ["shared"] -> print_string Esgg_shared.content
...
(*   | "input_direct"::tl -> map_query input_direct tl *)
  ...
  | ["tjson";"show";file] -> print_endline @@ Tjson.show @@ Tjson.parse @@ Std.input_file file
